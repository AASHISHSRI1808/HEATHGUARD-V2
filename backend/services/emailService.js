...
const sendEmail = async ({ to, subject, html }) => {
  await axios.post('https://api.brevo.com/v3/smtp/email', {
    sender: {
      name:  process.env.EMAIL_FROM_NAME || 'HealthGuard',
...
const emailService = {
  async sendPredictionReport(email, name, prediction) {
    ...
    <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #f8fafc;">
      ...
      <div style="padding: 30px; background: white;">
        ...
        <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; border-radius: 0 8px 8px 0; margin: 20px 0;">
          <p style="margin: 0; color: #92400e; font-size: 13px;">⚠️ This report is generated by AI and is not a substitute for professional medical advice. Please consult a qualified doctor for proper diagnosis and treatment.</p>
        </div>
    ...
    await sendEmail({ to: email, subject: `Health Guard – Your ${prediction.diseaseType} Disease Prediction Report`, html });
  },
  ...
  async sendAppointmentConfirmation(appointment) {
    ...
    await sendEmail({ to: patient.email, subject: 'Health Guard – Appointment Confirmation', html });
  },
  ...
  async sendDoctorApprovalEmail(email, name, status, reason = '') {
    ...
    await sendEmail({ to: email, subject: `Health Guard – Doctor Registration ${isApproved ? 'Approved' : 'Update'}`, html });
  },
};
